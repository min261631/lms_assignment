...
// ===== API helper (same-domain by default) =====
...
// Prisma/route.ts expects these exact enums
...
interface EscapeRoomPlayerProps {
  ...
  stages: Stage[];
  currentUserId: string;  // e.g. from session or a simple “pick player” flow
  roomId: string;         // EscapeRoom.id from DB so we can persist properly
}
...
const checkStageAnswer = (stage: Stage, input: string): { success: boolean; message: string } => {
  // 1. Normalize both inputs (simple answer check)
  ...
  // 2. Advanced Code Execution Simulation (preview only)
...
const EscapeRoomPlayer: React.FC<EscapeRoomPlayerProps> = ({
  roomConfig,
  stages,
  currentUserId,
  roomId,
  ...
  // Avoid duplicate "timeout fail" persistence
  ...
  // On timer expiration, persist a fail for current stage (once)
  useEffect(() => {
    if (timeRemaining === 0 && currentStage && !timeoutPersistedRef.current) {
      ...
      saveProgress({
        ...
        roomId,
        stageKey,
        submittedAnswer: studentAnswer || null,
        result: 'TIME EXPIRED',
        score: 0,
        status: 'INCORRECT',
        timeRemainingS: 0,
  ...
  }, [timeRemaining, currentStageIndex, currentStage, currentUserId, roomId, studentAnswer]);
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">
        <Card className="p-8 text-center bg-red-900/20 border-red-500">
          ...
          <CardDescription className="text-red-300">
            Cannot start preview: No puzzle stages defined.
          </CardDescription>
        </Card>
      </div>
    );
  ...
  // Persist on advance (pass/fail)
  const handleAdvanceStage = (result: { success: boolean; message: string }) => {
    setFeedback({ type: result.success ? 'success' : 'error', message: result.message });

    const stageKey = `stage-${currentStageIndex + 1}`;
    const score = result.success
      ? computeScore(stages.length, currentStageIndex, hintsRevealed, timeRemaining)
      : 0;

    // Map to enum expected by API
    const status: ProgressStatus = result.success ? 'CORRECT' : 'INCORRECT';
    ...
    if (result.success) {
      ...
      setTimeout(() => {
        if (currentStageIndex < stages.length - 1) {
          ...
          // Persist a final COMPLETED marker
          saveProgress({
            userId: currentUserId,
            roomId,
            stageKey: 'final',
            result: 'Room Completed',
            score: null,
            status: 'COMPLETED',
            timeRemainingS: timeRemaining,
          }).catch(() => {});
  ...
  const handleImageClick = () => {
    ...
    // Immediate CORRECT for click-debug
    handleAdvanceStage({
      success: true,
      message: '🔍 Clue found! Check the console (F12) for the hint. Advancing to the next lock.',
    });
  };
  ...
  // Optional – mark skip as "PENDING" (no score)
  const skipStage = () => {
    if (currentStageIndex < stages.length - 1) {
      const stageKey = `stage-${currentStageIndex + 1}`;
      saveProgress({
        ...
        <Card className="max-w-2xl w-full dark:bg-gray-800">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-base dark:text-gray-300">
              You successfully completed the room!
            </CardDescription>
          </CardHeader>
      ...
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <Card className="max-w-2xl w-full dark:bg-gray-800">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-base dark:text-gray-300">
              You ran out of time before completing all stages.
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
              ...
              <CardContent className="space-y-4">
                ...
                {currentStage.type === 'click-debug' ? (
                  <div className="text-center p-4">
                    <img
                      src={
                        currentStage.clueImage ||
                        roomConfig.backgroundImage ||
                        'https://placehold.co/300x300/4b5563/e5e7eb?text=Clue+Image'
                      }
                      alt="Clue Image"
                      ...
                      onError={(e) => {
                        (e.currentTarget as HTMLImageElement).src =
                          'https://placehold.co/300x300/4b5563/e5e7eb?text=Click+Me';
                      }}
                    />
                ...
                {currentStage.hints.length > 0 && (
                  <div>
                    ...
                    {currentStage.hints.slice(0, hintsRevealed).map((hint, index) => (
                      <Alert
                        key={index}
                        className="mt-2 bg-yellow-500/10 border-yellow-500/30 text-yellow-300"
                      >
            ...
            <Card className="dark:bg-gray-800">
              <CardHeader>
                ...
                <CardDescription className="dark:text-gray-400">
                  Enter your function or final key below.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <Textarea
                  ...
                  disabled={
                    currentStage.type === 'click-debug' || completedStages.has(currentStageIndex)
                  }
                />
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" onClick={skipStage}>
                    Skip
                  </Button>
                </div>
              </CardContent>
