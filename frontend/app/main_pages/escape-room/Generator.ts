...
// Helper to safely embed a JavaScript object into a JSON string within the HTML
const escapeJsData = (data: unknown) => JSON.stringify(data).replace(/\\/g, '\\\\').replace(/'/g, "\\'").replace(/\n/g, '\\n').replace(/\r/g, '\\r');
...
const generateJsValidationLogic = (puzzles: Stage[]): string => {
    // Escape and embed the puzzle data directly into the final JS
    const PUZZLES_JSON = escapeJsData(puzzles);

    return `
        // Data generated by the instructor's builder
        const PUZZLES = JSON.parse('${PUZZLES_JSON}');

        // Case-insensitive, whitespace-insensitive comparison utility
        function normalize(str) {
            if (typeof str !== 'string') return '';
            // Remove all whitespace, lowercase, and trim for strict comparison
        ...
        function checkAnswer(stageIndex, input) {
            ...
            if (puzzle.type === 'generate-numbers' || puzzle.type === 'port-data') {
                try {
                    ...
                    // The function definition is injected, and then the function is called.
                    const executionWrapper = new Function('testInput', \`
                        // ⚠️ WARNING: Running user-provided code (eval) 
                        \${userCode}; 
...
export function generateEscapeRoomHtml(config: RoomConfig, puzzles: Stage[]): string {
    ...
    const backgroundImage = config.backgroundImage || 'https://placehold.co/1200x800/111827/f3f4f6?text=Escape+Room+Background';
...
<head>
    ...
    <style>
        ...
        .timer { font-size: 2.5em; font-family: monospace; color: #10b981; font-weight: 900; }
        .stage-title { font-size: 1.5em; color: #facc15; margin-bottom: 15px; }
        .question { color: #e5e7eb; margin-bottom: 15px; white-space: pre-wrap; font-size: 1em; }
        .code-input { 
            width: 100%; 
            height: 180px; 
            padding: 15px; 
    ...
    <script>
        ...
        function renderStage(stageIndex) {
            ...
            if (puzzle.type === 'click-debug') {
                ...
                const buttonText = puzzle.question || 'Click the image to find the clue...';

                stageContent = \`
                    <div class="stage-title">Stage \${currentStageIndex + 1}: \${escapeHtml(puzzle.title)}</div>
                    <p class="question">\${escapeHtml(puzzle.description)}</p>
                    <div style="text-align: center; margin-top: 20px;">
                        ...
                        >
                        <button class="button" style="margin-top: 20px; background-color: #facc15; color: #111827;" onclick="handleDebugClick(\${currentStageIndex})">
        ...
        function showGameOver() {
            container.innerHTML = \`
                <div class="win-screen" style="background-color: #210d0d; border: 2px solid #ef4444;">
                    <h2 style="color: #ef4444; font-size: 3em; margin-bottom: 10px;">⏰ TIME EXPIRED!</h2>
                    <p style="font-size: 1.5em; color: #f87171;">The room is locked forever. Game Over.</p>
